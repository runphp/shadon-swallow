...
class Resource
{
    ...
    public static function placeHolders($mode)
    {
        $placeHolder = '';
        switch ($mode) {
            /*case 'mod':
    ...
    public static function getUri($strName)
    {
        ...
        if (isset(self::$arrMap[$strNamespace]) || self::register($strNamespace)) {
            ...
            if (isset($arrMap['res'][$strName])) {
                ...
                if ($arrRes['type'] == 'phtml') {
                    return self::$config['templateDir'] . $arrRes['uri'];
                } else {
            ...
            if ($type === 'css') {
                if (isset(self::$arrStaticCollection['css'])) {
                    ...
                    $html = '<link rel="stylesheet" type="text/css" href="' .
                         implode('"/>' . PHP_EOL . '    <link rel="stylesheet" type="text/css" href="', $arrURIs) . '"/>';
                }
                if (self::$styleArray) {
    ...
    public static function addScriptPool($str, $priority = 1)
    {
    ...
    public static function getResourceMap()
    {
        ...
        if (isset(self::$arrRequireAsyncCollection['res'])) {
            foreach (self::$arrRequireAsyncCollection['res'] as $id => $arrRes) {
                ...
                $arrResourceMap['res'][$id] = array('url' => $arrRes['uri']);
        ...
        if (isset(self::$arrRequireAsyncCollection['pkg']) && $needPkg) {
            foreach (self::$arrRequireAsyncCollection['pkg'] as $id => $arrRes) {
                $arrResourceMap['pkg'][$id] = array('url' => $arrRes['uri']);
            }
        }
    ...
    public static function register($strNamespace)
    {
        ...
        $arrConfigDir = self::$config['configDir']; //配置目录
                ...
                if (isset($arrMap['res'][$strName])) {
                    ...
                    if ($async && $arrRes['type'] === 'js') {
                        if ($arrPkg) {
                            ...
                            self::$arrRequireAsyncCollection['res'] = array_merge((array) self::$arrRequireAsyncCollection['res'], 
                                $arrPkgHas);
                        } else {
                        ...
                        $staticArr = $defaultDi->getConfig()->url->static->toArray();
                        $staticKey = array_rand($staticArr);
                        $static = $staticArr[$staticKey];
                        self::$arrStaticCollection[$arrRes['type']][] = $static . $strURI;
                    }
                    return $strURI;
                } else {
    ...
    {
        return self::setFramework(self::getUri($id));
    }
     ...
     */
    public function getPhtmlLoad($strName)
    {
        return self::load($strName);
    }
    ...
    {
        return self::renderResponse($html);
    }
    ...
    public function styleEnd()
    {
        ...
        if (preg_match($reg, $style, $matches)) {
            self::addStylePool($matches[2]);
        } else {
            self::addStylePool($style);
        }
    }
    ...
    public function scriptEnd()
    {
        ...
        if (preg_match($reg, $script, $matches)) {
            self::addScriptPool($matches[2]);
        } else {
            self::addScriptPool($script);
        }
    }
     ...
     * @return [type]       [description]
     */
    public function widget($id, $args = array())
    {
        $uri = self::getUri($id);
        if (is_file($uri)) {
            extract($args);
    ...
    function display($id, $array)
    {
        ...
        if (is_file($path)) {
            ...
            include $path;
            $html = ob_get_clean();
            self::load($id); //注意模板资源也要分析依赖，否则可能加载不全
            echo self::renderResponse($html);
        } else {
