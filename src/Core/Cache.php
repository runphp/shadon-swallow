...
class Cache
{
    ...
    private function setLink($prefix)
    {
        ...
        if (isset($this->cacheStore[self::$module][$prefix])) {
            if (isset($this->cacheStore[self::$module][$prefix]['store'])) {
                ...
                $this->cacheStore[self::$module][$prefix]['store'] == 'redis' && $serverConf['type'] = 'redis';
            }
        ...
        if (! isset($cacheServers[$key])) {
            $class = '\\Swallow\\Cache\\' . ucfirst($serverConf['type']);
            $serverConf['timeout'] = $serverConf['timeout'];
    ...
    public static function isSafeClear()
    {
        ...
        if ((isset($_GET['clear']) && $_GET['clear'] == 'cache' || Di::getInstance()->getShared('clientInfoNew')->getClearCache() == 'cache')
