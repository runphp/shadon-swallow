...
class Image
{
        ...
        $quality = 85)
    {
        ...
        $path = LOG_PATH . "imagelogs/image_error_" . date('Ymd', gmtime()) . ".txt"; //日志文件
        $text_lines = explode("\n", $waterText);
        $count_tmp = 0;
     ...
     * 添加图片水印
     *
     * @param $tmpUpFile   图片临时目录
     * @param $percent    压缩比例
     * @param $needToRarSize  大于指定大小才执行压缩
     * @param $imageType  转换后的图片类型
     * @author lanchuwei<lanchuwei@eelly.net>
     * @since  2015年11月10日
     */
    public static function imageRarSize($tmpUpFile, $percent, $needToRarSize = '',$imageType = '')
    {
        $fileSize = filesize($tmpUpFile);
        clearstatcache();
        if(empty($needToRarSize) || (!empty($needToRarSize) && $fileSize > $needToRarSize)){
            list ($width, $height, $type) = getimagesize($tmpUpFile); //获取原图尺寸
            //缩放尺寸
            $newwidth = $width * $percent;
            $newheight = $height * $percent;
            switch ($type) {
                case 1:
                    $srcIm = imagecreatefromgif($tmpUpFile);
                    break;
                case 2:
                    $srcIm = imagecreatefromjpeg($tmpUpFile);
                    break;
                case 3:
                    $srcIm = imagecreatefrompng($tmpUpFile);
                    break;
                default:
                    $srcIm = imagecreatefromjpeg($tmpUpFile);
                    break;
            ...
            imagecopyresized($dstIm, $srcIm, 0, 0, 0, 0, $newwidth, $newheight, $width, $height);
            if (!empty($imageType)){
                $tmpUpFile = preg_replace('/\.(\w+)/', '.'.$imageType, $tmpUpFile);
            }
            imagejpeg($dstIm,$tmpUpFile); //输出压缩后的图片
            imagedestroy($dstIm);
            imagedestroy($srcIm);
            return $tmpUpFile;
        }
