...
/**
 * 环信-服务器端REST API
 * @author    lanchuwei <lanchuwei@xiaoneimimi.com>
 */
...
class Easemob {
    ...
    public static function getInstance($options = null)
    {
        if (null === $options) {
            //读取默认配置信息
            $config = Conf::get('easemob');
            $options = $config['options'];
        }
        static $instance = [];
        if (! isset($instance['easemob'])) {
            $instance['easemob'] = new self($options);
        }
        return $instance['easemob'];
    }
    ...
    public function addFriend($owner_username, $friend_username) {
        ...
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        return $this->postCurl ( $url, '', $header );
    }
    /**
     * 删除好友
     *
     ...
     *            自定义参数
     */
    function yy_hxSend($from_user = "admin", $username, $content, $target_type = "users", $ext = NULL) {
     ...
     *            $toUser
     */
    public function setBlack($fromUser, $toUser) {
        $option ['usernames'] = [$toUser];
        $url = $this->url . "users/" . $fromUser .'/blocks/users';
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $result = $this->postCurl ( $url, $option, $header );
        return $result;
     ...
     *            $toUser
     */
    public function unsetBlack($fromUser, $toUser) {
        $url = $this->url . "users/" . $fromUser .'/blocks/users/' . $toUser;
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $result = $this->postCurl ( $url, [], $header , $type = 'DELETE');
        return $result;
    ...
    public function getStatus($user) {
        $url = $this->url . "users/" . $user .'/status';
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $result = $this->postCurl ( $url, [], $header , $type = 'GET');
        return $result;
...
//        fclose ( $fp );
     ...
     * @param int $passwd 用户密码
     */
    public function getUserToken($userName, $passwd) {
        $option ['grant_type'] = "password";
...
//         $expires_time = (int)$result ['expires_in'];
        $expires_time = 864000;
        $access_token = $result ['access_token'];
        $cacheKey = $userName.'_access_token';

        $cacheService = cache_server();
        $rs = $cacheService->set($cacheKey, $access_token, $expires_time);
